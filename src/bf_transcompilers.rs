...
pub struct Extension {
    ...
    pub function_callback: &'static str,
}
...
//TODO: Make sure that duplicate extensions don't load the file twice.
pub fn translate_rust(code: String, extensions: &Vec<Extension>) -> String
{
    ...
    let mut extension_header = String::from("//Extensions\n");
    for extension in extensions {
        let path = Path::new(extension.filename);
        ...
        file.read_to_string(&mut code).expect("Couldn't read file!");
        extension_header.push_str(&code);
        extension_header.push_str("\n");
    }
...
fn main() {
    ...
    for token in token_array {
        ...
        for extension in extensions {
            if c == extension.character {
                final_code.push_str(&"    ".repeat(max(indent - 1, 0)));
                final_code.push_str(extension.function_callback);
                final_code.push_str(";\n");
            }
        }
    }
    ...
    let result = extension_header + header + &final_code + footer;
